...
from sift.memory_forensics import vol3_effective_timeout as _vol3_eff_timeout
...
def _mobile_suspicion_class(mobile_signals: List[Dict[str, Any]],
    ...
    """
    try:
        from core.forensic_adapter import _EVIDENCE_MAP
        from tools.caie import _DOMAIN_MAP
    except Exception:
        return "GENERIC"
...
class SIFTOrchestrator:
    ...
    def analyze(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            from sift.sift_orchestrator import SIFTOrchestrator as _Real
            ...
            if pcap_path and not run_kwargs.get("network_flows"):
                try:
                    from sift.pcap_parser import parse_pcap_to_flows
            ...
            if mp:
                try:
                    from sift.mft_parser import parse_mft_file
    ...
    def _analyze_mobile(self, kwargs: Dict[str, Any]) -> list:
        ...
        if android_path:
            try:
                from sift.android_forensics import AndroidForensicsAnalyzer
        ...
        if ios_path:
            try:
                from sift.ios_forensics import iOSForensicsAnalyzer
        ...
        if takeout_path:
            try:
                from sift.google_takeout_forensics import GoogleTakeoutForensicsAnalyzer
        ...
        if macos_path:
            try:
                from sift.macos_forensics import MacOSForensicsAnalyzer
