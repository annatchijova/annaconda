...
#
# Architecture (motor not modified):
#   Motor result dict
#       ↓
#   extract_authorized_facts()  ← separate function, not part of evaluate()
#       ↓
...
#
# Run:
#   python3 annaconda/core/hallucination_guard.py  (runs __main__ test)
...
logger = logging.getLogger("annaconda.hallucination_guard")
...
def extract_authorized_facts(result: dict) -> frozenset:
    ...
    No dict flattening. No assumptions about unknown fields.
    ...
    Field paths covered:
      ...
      - result["fractures"][i]["severity"]
      - result["fractures"][i]["mitre_ttp"]
      - result["raw_scores"][i]["adjusted"]
      - result["raw_scores"][i]["raw_score"]
    ...
    # Scores — numeric match (str at the serialization boundary)
...
def match_fact(
    ...
    for fact in candidates:
        if fact.match_strategy == "exact":
            ...
            # guarantees no whitespace in canonical values).
            # Hashes: lower() only.
            if claim_type == "hash":
...
if __name__ == "__main__":
    ...
    # Simulate evaluate() output (scores are str)
    fake_result = {
        "verdict": "MALICE",
