...
from core.path_guard import PathGuard, SecurityException
from inference.abductive_reasoner import AbductiveReasoner

# SIFT motores originales
from sift._math_utils import (
    apply_artifact_reliability,
    apply_artifact_reliability_dynamic,
    build_redundancy_groups,
    process_all_groups,
)
from sift.memory_forensics import MemoryForensicsEngine, MemoryAnalysisResult
from sift.registry_timeline_reconstructor import RegistryTimelineReconstructor, RegistryAnalysisResult
from sift.event_log_correlator import EventLogCorrelator, EventLogAnalysisResult
from sift.disk_forensics import MFTTimelineAnalyzer, MFTAnalysisResult
from sift.network_forensics import NetworkForensicsEngine, NetworkAnalysisResult

# SIFT motores nuevos (con fallback si no existen todavía)
try:
    from sift.prefetch_analyzer import PrefetchAnalyzer, PrefetchAnalysisResult
except ImportError:
    PrefetchAnalyzer = None  # type: ignore
try:
    from sift.usb_device_tracker import USBDeviceTracker, USBAnalysisResult
except ImportError:
    USBDeviceTracker = None  # type: ignore
try:
    from sift.browser_forensics import BrowserForensicsEngine, BrowserAnalysisResult
except ImportError:
    BrowserForensicsEngine = None  # type: ignore
try:
    from sift.shellbag_analyzer import ShellbagAnalyzer, ShellbagAnalysisResult
except ImportError:
    ShellbagAnalyzer = None  # type: ignore
try:
    from sift.amcache_shimcache import AmcacheShimcacheAnalyzer, AmcacheAnalysisResult
except ImportError:
    AmcacheShimcacheAnalyzer = None  # type: ignore
try:
    from sift.ioc_manager import IOCManager, IOCMatchResult
except ImportError:
    IOCManager = None  # type: ignore
try:
    from sift.unified_timeline_engine import UnifiedTimelineEngine, TimelineAnalysisResult
except ImportError:
...
try:
    from inference.metabolic_profiler import MetabolicProfiler, MetabolicAnalysisResult
except ImportError:
    MetabolicProfiler = None  # type: ignore
try:
    from inference.cross_artifact_resonance import CrossArtifactResonance, ResonanceResult
except ImportError:
    CrossArtifactResonance = None  # type: ignore
try:
    from inference.behavioral_fingerprint import BehavioralFingerprint, BehavioralFingerprintResult
except ImportError:
    BehavioralFingerprint = None  # type: ignore
try:
    from inference.case_pattern_library import CasePatternLibrary, CasePatternResult
except ImportError:
...
class SIFTOrchestrator:
    ...
    def run_full_analysis(
        ...
        caie_result = None
        try:
            from core.forensic_adapter import ForensicAdapter
